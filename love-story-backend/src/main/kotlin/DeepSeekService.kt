...
@Serializable data class DeepSeekMessage(val role: String, val content: String)

@Serializable data class DeepSeekRequest(val model: String, val messages: List<DeepSeekMessage>)

@Serializable data class DeepSeekChoice(val message: DeepSeekMessage)

@Serializable data class DeepSeekResponse(val choices: List<DeepSeekChoice>)
            ...
            val response: DeepSeekResponse =
                    client
                            .post(DEEPSEEK_API_URL) {
                                ...
                                val requestBody =
                                        DeepSeekRequest(
                                                model = "deepseek-chat", // 或者使用
                                                // deepseek-coder，根据你的需求
                                                messages =
                                                        listOf(
                                                                DeepSeekMessage(
                                                                        role = "user",
